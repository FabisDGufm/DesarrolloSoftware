...
describe('Explore Routes Integration Tests', () => {
    ...
    describe('GET /explore?q=...', () => {
        it('should return 200 when searching with query', async () => {
            const response = await request(app).get('/explore').query({ q: 'Alice' }).expect(200);
    ...
    describe('GET /explore/search', () => {
        it('should return 200 and search results', async () => {
            const response = await request(app).get('/explore/search').query({ q: 'post' }).expect(200);
            expect(response.body.status).toBe('success');
            expect(Array.isArray(response.body.data)).toBe(true);
        });
    });
    ...
    describe('GET /explore/browse/:type', () => {
        it('should return 200 for valid type', async () => {
            const response = await request(app).get('/explore/browse/post').expect(200);
            expect(response.body.status).toBe('success');
            expect(Array.isArray(response.body.data)).toBe(true);
        });

        it('should return 400 for invalid type', async () => {
            const response = await request(app).get('/explore/browse/invalid').expect(400);
